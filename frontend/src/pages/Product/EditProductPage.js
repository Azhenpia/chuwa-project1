...
import { useFetchProductsQuery, useUpdateProductMutation } from '../../features/api/apiSlice'; // **绿色** 修改为 useUpdateProductMutation
import "../../styles/Product.css";
import { useLocation } from 'react-router-dom';

export default function EditProductPage() { // **黄色** 修改名称更贴合功能
  ...
  const handleSubmit = async (e) => { // **绿色** 将函数改为异步
    e.preventDefault();
    const newErrors = {};
    ...
    if (!hasErrors) {
      try {
        console.log("Submitting form:", formData); // **绿色** 调试信息
        const response = await updateProduct({ id: product.id, ...formData }).unwrap(); // **绿色** 调用 Mutation
        console.log("Product updated successfully:", response); // **绿色** 成功提示
      } catch (error) {
        console.error("Failed to update product:", error); // **绿色** 错误提示
      }
    }
  };
  ...
  return (
    <div className="create-product-page">
      <div className="product-form">
        <h1>Edit Product</h1> {/* **黄色** 修改标题 */}
        <p>商品id: {JSON.stringify(product)}</p>
        <p>formData是: {JSON.stringify(formData)}</p>
        {isUpdating && <p>Updating product...</p>} {/* **绿色** 显示更新状态 */}
        {updateError && <p style={{ color: "red" }}>Error: {updateError.message}</p>} {/* **绿色** 显示错误信息 */}
